...
int client() { 
    ...
    sendto(sockfd, packet_stream(packet), packet->size, 0, (struct sockaddr*)&server_addr, server_struct_length);
    ...
    while (1) {
        if(select(sockfd + 1, &rfds, NULL, NULL, &tv)) {
            if (FD_ISSET(STDIN_FILENO, &rfds)) {
                ...
                memset(packet->content, 0, packet->content_size);
                recvfrom(sockfd, buffer, packet->size, 0, (struct sockaddr*)&server_addr, &server_struct_length);
            ...
            struct sockaddr_in fresh_server_addr;
    ...
    while (1) {
        if(select(sockfd + 1, &rfds, NULL, NULL, &tv)) {
            if (FD_ISSET(STDIN_FILENO, &rfds)) {
                memset(packet->content, 0, packet->content_size);
                read(STDIN_FILENO, packet->content, packet->content_size);
                sendto(sockfd, packet_stream(packet), packet->size, 0, (struct sockaddr*)&client_addr, client_struct_length);
                FD_SET(sockfd, &rfds);
            } else {
                memset(packet->content, 0, packet->content_size);
                recvfrom(sockfd, buffer, packet->size, 0, (struct sockaddr*)&client_addr, &client_struct_length);
                packet_parse(packet, buffer);
                if (strcmp(packet->content, NTP_DEFAULT_CONTENT)) {
                    write(STDOUT_FILENO, packet->content, packet->content_size);
                    fflush(stdout);
